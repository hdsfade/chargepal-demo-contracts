...
contract ChargeRewardClaim is OwnableUpgradeable {
    using SafeERC20 for IERC20;

    IERC20 public charge;
    IERC721 public chargeNFT;
    ...
    function claim(uint256 _id, uint256 _quota, uint256 _amount, bytes32[] calldata _proof) external {
        address msgSender = msg.sender; // address of claimee

        if (msgSender != chargeNFT.ownerOf(_id)) revert NonNFTOwner();

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(_id, _quota));
